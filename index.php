...
// Submit Idea in Brainstorm
if (isset($_POST['action']) && $_POST['action'] === 'submit_idea' && $sessionId && $currentSession['method'] === 'brainstorm' && !empty($_POST['idea_text'])) {
    $ip = $_SERVER['REMOTE_ADDR'];
    $geo = getGeoInfo($ip);
    $currentSession['ideas'][] = [
        'text' => htmlspecialchars(trim($_POST['idea_text'])),
        'time' => time(),
        'ip' => $ip,
        'geo' => $geo
    ];
...
if ($sessionId && $currentSession) {
    if (isset($_GET['admin'])) {
        ...
        if ($currentSession['method'] === 'poll') {
            $viewMode = 'poll_vote';
        } elseif ($currentSession['method'] === 'brainstorm') {
            $viewMode = 'brainstorm';
        } else {
            $viewMode = $isAuthenticated ? 'dashboard' : 'login';
        }
...
<body class="bg-gradient min-h-screen text-slate-100 p-4">
    ...
    <div class="max-w-md mx-auto">
        ...
        <?php elseif ($viewMode === 'brainstorm'): ?>
            ...
            <section class="bg-slate-800/50 backdrop-blur-md p-6 rounded-2xl border border-slate-700 shadow-xl">
                <div class="text-center mb-6">
                    <h2 class="text-2xl font-bold text-white"><?= htmlspecialchars($currentSession['title']) ?></h2>
                    <p class="text-slate-400 text-sm mt-1">Teile deine Ideen!</p>
                </div>

                <form method="POST" class="space-y-4 mb-8">
                    <input type="hidden" name="action" value="submit_idea">
                    <textarea name="idea_text" required placeholder="Deine Idee..." class="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all min-h-[100px] text-slate-100"></textarea>
                    <button type="submit" class="w-full bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-3 rounded-xl transition-all transform active:scale-[0.98]">
                        Idee einreichen
                    </button>
                </form>

                <div class="space-y-4">
                    <h3 class="text-xs font-black uppercase tracking-widest text-slate-500">Bisherige Ideen</h3>
                    <?php if (empty($currentSession['ideas'])): ?>
                        <p class="text-slate-500 text-sm italic">Noch keine Ideen vorhanden.</p>
                    <?php else: ?>
            ...
            <main class="space-y-6">
                ...
                <?php if ($currentSession['method'] === 'brainstorm'): ?>
                    <section class="bg-slate-800/30 p-6 rounded-2xl border border-slate-700/50 space-y-4">
                        <h3 class="text-xs font-black uppercase tracking-widest text-slate-500">Eingereichte Ideen (Admin)</h3>
                        <div class="space-y-4">
                            <?php if (empty($currentSession['ideas'])): ?>
                                <p class="text-slate-500 text-sm italic">Noch keine Ideen vorhanden.</p>
                            <?php else: ?>
                                <?php foreach ($currentSession['ideas'] as $idea): ?>
                                    <div class="bg-slate-900/50 p-4 rounded-xl border border-slate-700/50">
                                        <p class="text-slate-200"><?= nl2br(htmlspecialchars($idea['text'])) ?></p>
                                        <div class="text-[9px] text-slate-600 mt-2 font-mono">
                                            <?= date('d.m.Y H:i:s', $idea['time']) ?> | IP: <?= $idea['ip'] ?> | Loc: <?= $idea['geo']['city'] ?>, <?= $idea['geo']['country'] ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                ...
                <!-- Result Display (Non-Poll/Non-Brainstorm) -->
                <?php if ($selectionResult && !in_array($currentSession['method'], ['poll', 'brainstorm'])): ?>
                ...
                <!-- Options List (Not for Brainstorm) -->
                <?php if ($currentSession['method'] !== 'brainstorm'): ?>
                ...
                <?php elseif ($currentSession['method'] === 'brainstorm'): ?>
                    <div class="mt-8 pt-4 border-t border-slate-700/50 text-center">
                        <a href="?id=<?= $sessionId ?>" class="text-[10px] text-slate-500 hover:text-cyan-400 uppercase tracking-widest font-bold transition-colors">Zur Brainstorm-Ansicht (für Teilnehmer)</a>
                    </div>
                <?php endif; ?>
            </main>
        <?php endif; ?>
    ...
    <script>
        function copyLink() {
            // Copy voting/brainstorm link by default
